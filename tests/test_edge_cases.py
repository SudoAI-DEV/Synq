...
from synq.core.snapshot import ColumnSnapshot, SchemaSnapshot, SnapshotManager, TableSnapshot
...
def test_migration_namer_edge_case_sanitization():
    ...
    operations = [Mock()]
    operations[0].operation_type = Mock()
    operations[0].operation_type.value = "create_table"
    operations[0].table_name = "___---!!!"
    operations[0].object_name = None
    
    with patch.object(namer, '_analyze_operations') as mock_analyze:
        mock_analyze.return_value = {
            'create_table_count': 1,
            'table_names': {'___---!!!'},
            'primary_operation': 'create_table'
        }
...
def test_migration_namer_analyze_operations():
    ...
    assert analysis['create_table_count'] == 2
    assert analysis['add_column_count'] == 1
...
def test_migration_namer_get_primary_operation():
    ...
    primary = namer._get_primary_operation(operations)
    assert primary == OperationType.CREATE_TABLE
...
def test_migration_namer_sanitize_name():
    ...
    assert namer._sanitize_name("123_starts_with_number") == "starts_with_number"
    assert namer._sanitize_name("___only_underscores___") == "only_underscores"
    assert namer._sanitize_name("") == ""
...
def test_migration_namer_generate_table_creation_name():
    ...
    name = namer._generate_table_creation_name(operations, {"user_profiles"})
    assert "create" in name.lower()
    assert "profile" in name.lower()
...
def test_migration_namer_generate_column_name():
    ...
    name = namer._generate_column_name(operations, {"users"})
...
def test_migration_namer_generate_mixed_name():
    ...
    name = namer._generate_mixed_name(operations, {"users"})
    ...
    tables = [
        TableSnapshot(
            ...
            columns=[
                ...
                ColumnSnapshot(name="created_at", type="DATETIME", nullable=False, default="CURRENT_TIMESTAMP")
            ],
            indexes=[],
            foreign_keys=[]
        )
