...
def test_migration_manager_get_all_migrations():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        (manager.migrations_path / "0001_first.sql").write_text("CREATE TABLE first (id INTEGER);")
        (manager.migrations_path / "0002_second.sql").write_text("CREATE TABLE second (id INTEGER);")
...
def test_migration_manager_get_pending_migrations():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        applied_migrations = ["0001_first.sql"]
        
        pending = manager.get_pending_migrations(applied_migrations)
...
def test_migration_manager_get_migration_by_number():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Create migration files
        (manager.migrations_path / "0001_first.sql").write_text("CREATE TABLE first (id INTEGER);")
        (manager.migrations_path / "0003_third.sql").write_text("CREATE TABLE third (id INTEGER);")
