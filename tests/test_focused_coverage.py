...
def test_migration_manager_get_all_migrations_parsing():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        (manager.migrations_path / "0001_create_users.sql").write_text("CREATE TABLE users;")
...
def test_migration_manager_get_pending_migrations_basic():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        mock_db_manager.get_applied_migrations.return_value = ["0001_first.sql", "0002_second.sql"]
...
def test_migration_namer_sanitize_name():
    ...
    assert namer._sanitize_name("___test___") == "test"  # Remove leading/trailing underscores
...
def test_migration_namer_multi_table_name():
    ...
    assert "create" in name.lower() or "tables" in name.lower() or "initial" in name.lower()
