...
def test_init_command_error_handling():
    ...
    with patch('synq.core.config.SynqConfig') as mock_config:
        mock_config.side_effect = Exception("Config creation failed")
        
        with patch('click.echo') as mock_echo:
            with pytest.raises(SystemExit):  # click.Abort raises SystemExit
                init_command(
            ...
            mock_echo.assert_any_call("❌ Error initializing Synq: Config creation failed", err=True)
...
def test_generate_command_successful_generation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch('synq.utils.import_utils.import_metadata_from_path', return_value=metadata):
            with patch('synq.utils.import_utils.validate_metadata_object'):
                with patch('synq.core.migration.MigrationManager') as mock_manager:
                    with patch('synq.core.snapshot.SnapshotManager') as mock_snapshot:
                        with patch('synq.core.naming.generate_migration_name', return_value="create_users_table"):
                            # Mock the managers
                            ...
                            mock_mgr_instance.detect_changes.return_value = [Mock()]  # Non-empty operations
                            mock_mgr_instance.create_migration_name.return_value = "create_users_table"
                            mock_snap_instance.get_next_migration_number.return_value = 1
                            mock_mgr_instance.generate_sql.return_value = "CREATE TABLE users (id INTEGER);"
                            mock_mgr_instance.save_migration.return_value = Path("0001_create_users_table.sql")
                            mock_snap_instance.save_snapshot.return_value = Path("0001_snapshot.json")
                            ...
                            with patch('click.echo') as mock_echo:
                                ...
                                mock_echo.assert_any_call("📦 Loading SQLAlchemy metadata...")
                ...
                with patch('synq.core.migration.MigrationManager') as mock_manager:
                    with patch('synq.core.snapshot.SnapshotManager') as mock_snapshot:
                        ...
                        with patch('click.echo') as mock_echo:
                            ...
                            mock_echo.assert_any_call("✅ No schema changes detected. Nothing to migrate!")
            ...
            with patch('synq.utils.import_utils.validate_metadata_object'):
                with patch('synq.core.migration.MigrationManager') as mock_manager:
                    with patch('synq.core.snapshot.SnapshotManager') as mock_snapshot:
                        ...
                        mock_mgr_instance.create_migration_name.return_value = "custom_migration"
                        mock_snap_instance.get_next_migration_number.return_value = 1
                        mock_mgr_instance.generate_sql.return_value = "CREATE TABLE users (id INTEGER);"
                        mock_mgr_instance.save_migration.return_value = Path("0001_custom_migration.sql")
                        mock_snap_instance.save_snapshot.return_value = Path("0001_snapshot.json")
                        ...
                        with patch('click.echo') as mock_echo:
                            ...
                            mock_echo.assert_any_call("🏷️  Using custom migration name: custom_migration")
...
def test_migrate_command_successful():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch('synq.core.database.DatabaseManager') as mock_db:
            with patch('synq.core.migration.MigrationManager') as mock_migration:
                ...
                mock_mgr_instance.get_pending_migrations.return_value = [
                    Mock(filename="0001_test.sql", sql_content="CREATE TABLE test (id INTEGER);")
                ]
                ...
                with patch('synq.core.database.DatabaseManager') as mock_db:
                    with patch('synq.core.migration.MigrationManager') as mock_migration:
                        with patch('synq.core.snapshot.SnapshotManager') as mock_snapshot:
                            ...
                            mock_db_instance.get_applied_migrations.return_value = ["0001_initial.sql"]
                            ...
                            with patch('click.echo') as mock_echo:
                                ...
                                mock_echo.assert_any_call("📊 Migration Status")
                    ...
                    with patch('synq.core.migration.MigrationManager') as mock_migration:
                        with patch('synq.core.snapshot.SnapshotManager') as mock_snapshot:
                            ...
                            with patch('click.echo') as mock_echo:
                                ...
                                mock_echo.assert_any_call("⚠️  Pending schema changes detected:")
            ...
            with patch('synq.utils.import_utils.validate_metadata_object'):
                with patch('synq.core.database.DatabaseManager') as mock_db:
                    ...
                    with patch('click.echo') as mock_echo:
                        ...
                        mock_echo.assert_any_call("❌ Cannot connect to database")
...
def test_command_error_handling():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch('click.echo') as mock_echo:
            ...
            mock_echo.assert_any_call("❌ Error generating migration: Could not import module 'nonexistent.module': No module named 'nonexistent'", err=True)
