...
def generate_command(
    ...
    try:
        ...
        # Import metadata
        click.echo("📦 Loading SQLAlchemy metadata...")
        ...
        click.echo("📸 Creating current schema snapshot...")
        ...
        if not migration_ops:
            click.echo("✅ No schema changes detected. Nothing to migrate!")
            return

        # Generate migration name
        if custom_name:
            migration_name = migration_manager.create_migration_name(custom_name)
            click.echo(f"🏷️  Using custom migration name: {custom_name}")
        elif description:
            migration_name = migration_manager.create_migration_name(description)
            click.echo(f"🏷️  Using provided description: {description}")
        else:
            ...
            click.echo(f"🤖 Auto-generated migration name: {auto_name}")
        ...
        click.echo(f"✅ Created snapshot: {snapshot_file}")
        click.echo("")
